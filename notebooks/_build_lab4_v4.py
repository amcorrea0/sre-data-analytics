"""Reescribe lab4_ejecutivo como dashboard ejecutivo presentable:
- 4 KPIs uno por seccion
- Recomendaciones como 'especialista' (no solo RICE abstracto)
- Headline lista para presentar
"""
...
LAB_4 = notebook([
    md("# Lab 4 - Dashboard ejecutivo: decisiones con datos\n\n"
       "**Modalidad:** hands-on (en parejas) - **Duracion:** ~12 min\n\n"
       "## El caso de negocio\n\n"
       ...
       "Este dashboard es la respuesta. 4 KPIs uno por seccion, "
       "cada uno con una recomendacion como especialista."),
    ...
    md("## 1) Disponibilidad y SLOs - ¿estamos cumpliendo lo prometido?\n\n"
    ...
    py("avail = 1 - metrics['error_rate'].mean()\n"
       ...
       "print(f'SLO objetivo: 99.9%')\n"
       "print(f'Cumplimos SLO: {avail >= 0.999}')\n\n"
    ...
    py("auto_ok = (runs['status'] == 'success').mean()\n"
       ...
       "mttr_per_job = failed.groupby('job_name')['duration_sec'].median() / 60  # minutos\n"
       "worst_job = mttr_per_job.sort_values(ascending=False).head(1)\n"
       "print(f'Success rate global: {auto_ok:.2%}')\n"
       "print(f'Job con peor MTTR: {worst_job.index[0]} ({worst_job.iloc[0]:.1f} min)\\n')\n\n"
       "fig, ax = plt.subplots(figsize=(12, 5))\n"
    ...
    md("## 4) Recomendacion RICE - que hacer primero\n\n"
       "**Que decidimos:** priorizamos las acciones donde RICE = (Reach x Impact) / "
       "Effort es mayor."),

    py("recos = pd.DataFrame([\n"
       "    {'accion': 'Comprar capacidad para servicio saturado', 'reach': 9, 'impact': 4, 'effort': 3},\n"
       ...
       "    {'accion': 'Implementar auto-scaling en horas pico', 'reach': 8, 'impact': 2, 'effort': 1},\n"
       "])\nrecos['rice'] = recos['reach'] * recos['impact'] / recos['effort']\nrecos.sort_values('rice', ascending=False)"),

    md("## 5) Decisiones como especialista (no solo datos)\n\n"
       "Mirando los datos y mi experiencia operativa, estas son las recomendaciones "
       "que llevaria al CFO:\n\n"
       "| # | Tema | Recomendacion |\n"
       "|---|------|---------------|\n"
       "| 1 | **Capacidad** | Comprar nodos para servicio con `CPU p95 > 80%` con "
       "reserva anual (ahorra 30% vs on-demand). |\n"
       "| 2 | **Confiabilidad** | Si disponibilidad < 99.9%, invertir el 20% del tiempo "
       "del equipo en reducir incidentes recurrentes. |\n"
       "| 3 | **Automatizacion** | Reescribir el job con peor MTTR. Cada hora que se "
       "demora un job = N retries = N tiempo de operador. |\n"
       "| 4 | **Operacion** | Rotar turnos lunes-viernes 9-11am para absorber la "
       "demanda visible en los heatmaps (en vez de contratar mas gente). |\n"
       "| 5 | **Cambios** | Si rollback > 15%, hacer postmortem del proceso de "
       "cambios antes de tocar mas infraestructura. |\n\n"
       "**No compres tecnologia si antes no eliminaste lo que esta roto.**"),

    md("## 6) Headline ejecutiva final\n\n"
       "Tu presentacion al CFO en 4 frases (1 por seccion):\n\n"
       "**Disponibilidad:** «Disponibilidad observada del **X%**; cumplimos "
       "SLO de 99.9% [Si/No]. El top 3 de incidentes impacto a **N** clientes.»\n\n"
       "**Automatizacion:** «Success rate global del **X%**; el job **[NOMBRE]** "
       "es responsable del **Y%** de las fallas con MTTR de **Z** min.»\n\n"
       "**Operacion:** «Atendemos **X** tickets/dia con un breach SLA promedio "
       "del **Y%**; el equipo **[NOMBRE]** lidera con **Z%**.»\n\n"
       "**Recomendacion prioritaria:** «La accion con mayor RICE es **[ACCION]**; "
       "cuesta **X** sprints, evita **Y** horas/año, recupera **Z**% del burn rate.»"),
])
